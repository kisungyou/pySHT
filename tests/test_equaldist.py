...
class TestBiswasGhoshTwoSample(unittest.TestCase):
    ...
    def test_identical_constant_samples_have_zero_statistic(self) -> None:
        x = np.zeros((5, 3))
        y = np.zeros((7, 3))
        result = bg_2samp(x, y, n_resamples=39, rng=7)
    ...
    def test_clear_distributional_alternative_has_small_pvalue(self) -> None:
        x = np.zeros((8, 2))
        y = np.full((8, 2), 10.0)
        result = bg_2samp(x, y, n_resamples=499, rng=12)
    ...
    def test_integer_seed_is_deterministic(self) -> None:
        first = bg_2samp(self.x, self.y, n_resamples=79, rng=2026)
        second = bg_2samp(self.x, self.y, n_resamples=79, rng=2026)
        self.assertEqual(first, second)

    def test_row_order_does_not_change_statistic_or_calibration(self) -> None:
        baseline = bg_2samp(self.x, self.y, n_resamples=99, rng=101)
        reordered = bg_2samp(
            self.x[[3, 0, 4, 1, 2]],
            self.y[[5, 2, 0, 4, 1, 3]],
            n_resamples=99,
    ...
    def test_swapping_groups_does_not_change_result(self) -> None:
        xy = bg_2samp(self.x, self.y, n_resamples=99, rng=42)
        yx = bg_2samp(self.y, self.x, n_resamples=99, rng=42)
    ...
    def test_plus_one_pvalue_is_nonzero_and_in_range(self) -> None:
        x = np.zeros((8, 1))
        y = np.full((8, 1), 100.0)
        result = bg_2samp(x, y, n_resamples=19, rng=8)
    ...
    def test_common_rescaling_preserves_normalized_inference(self) -> None:
        x = np.zeros((8, 1))
        y = np.ones((8, 1))
        baseline = bg_2samp(x, y, n_resamples=199, rng=5)
        ...
        moderate = bg_2samp(3.0 * x, 3.0 * y, n_resamples=199, rng=5)
    ...
    def test_rotation_preserves_theoretical_permutation_ties(self) -> None:
        ...
        baseline = bg_2samp(x, y, n_resamples=999, rng=11)
        rotated = bg_2samp(
            x @ rotation,
            y @ rotation,
            n_resamples=999,
    ...
    def test_result_is_immutable_and_has_htest_style_display(self) -> None:
        result = bg_2samp(self.x, self.y, n_resamples=19, rng=2)
    ...
    def test_rejects_invalid_samples(self) -> None:
        invalid_calls = (
            lambda: bg_2samp([1.0], [1.0, 2.0], n_resamples=3),
            lambda: bg_2samp(np.ones((3, 2)), np.ones((3, 3)), n_resamples=3),
            lambda: bg_2samp([0.0, np.nan], [1.0, 2.0], n_resamples=3),
            lambda: bg_2samp(np.ones((2, 2, 2)), np.ones((2, 2)), n_resamples=3),
            lambda: bg_2samp([True, False], [False, True], n_resamples=3),
            lambda: bg_2samp([1.0 + 1.0j, 2.0], [1.0, 2.0], n_resamples=3),
        )
    ...
    def test_rejects_invalid_resampling_controls(self) -> None:
        for invalid in (0, -1):
            with self.subTest(n_resamples=invalid), self.assertRaises(ValueError):
                bg_2samp(self.x, self.y, n_resamples=invalid)
    ...
    def test_asymptotic_and_unknown_calibrations_are_not_silently_used(self) -> None:
        ...
        with self.assertRaises(ValueError):
            bg_2samp(self.x, self.y, calibration="bootstrap", n_resamples=3)
    ...
    def test_monte_carlo_correction_and_uncertainty_are_explicit(self) -> None:
        ...
        tail_estimate = result.exceedances / 29.0
        expected_se = np.sqrt(29.0 * tail_estimate * (1.0 - tail_estimate)) / 30.0
